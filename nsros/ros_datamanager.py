...
class ROSDataManager(
    ...
    train_dataset: Union[ROSDataset]
    ...
    def setup_train(self):
        ...
        config_kwargs = vars(self.config).copy()  # Assuming self.config is an object with attributes
# If self.config is a dictionary, use: config_kwargs = self.config.copy()
        config_kwargs.pop('device', None)  # Remove 'device' key if exists
        # print(f"Device argument: {self.config.device}")
...
class ROSFullImageDataManager(ROSDataManager, Generic[TDataset]):
    def setup_train(self):
        ...
        """
        assert self.train_dataset is not None
        # print(f"Device argument: {self.device} -----------------------------------")
        self.train_image_dataloader = ROSDataloader(
            self.train_dataset,
            self.config.data_update_freq,
            device=self.device,
        ...
        print(f"Device argument: {self.device} EnD")
